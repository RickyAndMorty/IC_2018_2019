...
void resultados::gravarSnir(double** matriz, int linhas, int colunas)
{
    FILE *pArq;
    int i,j;
    if((pArq = fopen("C:/git/IC_2018_2019/OTIMIZACAO_OCDMA_32x64/resultados/snir.bin", "wb")) == 0x0)
    {
        printf("erro.");
        exit(1);
    }
...
void resultados::gravarPgbest(double** pgbest, int iterations)
{
    FILE *pArq;
    int i,j;
    if((pArq = fopen("C:/git/IC_2018_2019/OTIMIZACAO_OCDMA_32x64/resultados/Pgbest.bin", "wb")) == 0x0)
    {
        printf("erro.");
        exit(1);
    }
...
void resultados::gravarSnirdB(double* _snir, int users)
{
    FILE *pArq;
    int i;
    if((pArq = fopen("C:/git/IC_2018_2019/OTIMIZACAO_OCDMA_32x64/resultados/SNIRdB.bin", "wb")) == 0x0)
    {
        printf("erro.");
        exit(1);
    }
...
void resultados::gravarPower(double* _power, int users)
{
    FILE *pArq;
    int i;
    if((pArq = fopen("C:/git/IC_2018_2019/OTIMIZACAO_OCDMA_32x64/resultados/power.bin", "wb")) == 0x0)
    {
        printf("erro.");
        exit(1);
    }
...
{
    FILE *pArq;
    if((pArq = fopen("C:/git/IC_2018_2019/OTIMIZACAO_OCDMA_32x64/resultados/constantes.txt", "w")) == 0x0)
    {
        printf("erro.");
        exit(1);
    }
